...
class SkinMylib extends SkinTemplate {
	...
	public function initPage( OutputPage $out ) {
		global $wgStylePath, $wgJsMimeType, $wgStyleVersion, $wgScriptPath;

		parent::initPage( $out );

		// Append skin-specific styles
		$out->addStyle( 'mylib/main-rtl.css', 'screen', '', 'rtl' );
		$out->addStyle( 'mylib/main-ltr.css', 'screen', '', 'ltr' );
		...
		$out->addScript(
			'<!--[if lt IE 7]><style type="text/css">body{behavior:url("' .
				$wgStylePath .
				'/mylib/csshover.htc")}</style><![endif]-->'
		);
	}
	/**
 ...
 * @ingroup Skins
 */
class MylibTemplate extends QuickTemplate {
